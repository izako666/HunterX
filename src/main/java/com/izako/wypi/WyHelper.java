...
public class WyHelper
{
	...
	public static BlockRayTraceResult rayTraceBlocks(Entity source)
	{
		...
		double d1 = (source.prevPosY + (source.getPosY() - source.prevPosY) * f + 1.6200000000000001D) - source.getYOffset();
		...
		for (Block b : blocks)
		{
			for (int x = -radius; x <= radius; x++)
				for (int y = -radius; y <= radius; y++)
					...
					{
						if (player.world.getBlockState(new BlockPos((int) player.getPosX() + x, (int) player.getPosY() + y, (int) player.getPosZ() + z)).getBlock() == b)
						{
							return true;
						}
					}
...
*/
	...
	{
		GlStateManager.rotatef(180.0F + headYawOffset, 0.0F, 1.0F, 0.0F);
