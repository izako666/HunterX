...
public class EntityInit {
	...
	public static void registerEntities() {
		...
		EntityRegistry.registerModEntity(new ResourceLocation(Reference.MOD_ID + ":" + "EntityNeedle"),
				EntityNeedle.class, "EntityCard", 125, Main.instance, 64, 10, true);
		EntityRegistry.registerModEntity(new ResourceLocation(Reference.MOD_ID + ":" + "EntityCard"),
				YoyoProjectile.class, "YoyoProjectile", 126, Main.instance, 64, 10, true);
		EntityRegistry.registerModEntity(new ResourceLocation(Reference.MOD_ID + ":" + "EntityBullet"),
				EntityBullet.class, "EntityBullet", 127, Main.instance, 64, 10, true);
	...
	private static void copySpawns(final Class<? extends EntityLiving> classToAdd,
			final EnumCreatureType creatureTypeToAdd, final Class<? extends EntityLiving> classToCopy,
			final EnumCreatureType creatureTypeToCopy, int spawnrate, int MaxInGroup) {
		for (final Biome biome : ForgeRegistries.BIOMES) {
			biome.getSpawnableList(creatureTypeToCopy).stream().filter(entry -> entry.entityClass == classToCopy)
					.findFirst().ifPresent(spawnListEntry -> biome.getSpawnableList(creatureTypeToAdd)
